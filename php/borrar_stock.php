...
$importe_restar = floatval($quantity) * floatval($precio_unitario);
echo $importe_restar;

//HACEMOS EL BORRADO DEFINITIVO DEL STOCK
$sql_delete = "DELETE FROM stocks WHERE id_stocks = $id_stock";
if (!mysqli_query($conexion, $sql_delete)) {
    responder(null, false, "Error al borrar stock: " . mysqli_error($conexion), $conexion);
    exit;
}
...
echo $sql_update_wallet;
if (!mysqli_query($conexion, $sql_update_wallet)) {
    responder(null, false, "Error al actualizar wallet: " . mysqli_error($conexion), $conexion);
    exit;
}

responder(null, true, "Stock borrado y wallet actualizada correctamente", $conexion);
?>